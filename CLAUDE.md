...
- `agents/*.md` -- Agent definitions (YAML frontmatter: `name`, `description`, `model`, `effort`, `color`; no `tools` — agents inherit every tool)
...
- Every agent pins `model` and `effort` in frontmatter, matched to task shape (model = how hard the task is; effort = how far the agent travels — files read, verification depth). Current tiers: opus+high for generative full tracks (code-architect, implementer, trimmer) and for judgment-heavy review of scoped input (code-reviewer, red-team, plan-doc-reviewer, silent-failure-hunter, code-simplifier); sonnet+high for scoped single-dimension legs where travel is the job (code-explorer, spec-reviewer, test-analyzer, type-analyzer) and for near-mechanical checks (comment-analyzer). There are currently 13 agents.
- Agents don't pin `tools`: with the field omitted, an agent inherits every tool available to subagents, MCP included. Advisory agents never edit code — they report findings; source is modified only by implementer and by code-simplifier in direct mode.
- `allowed-tools` on a slash-invoked skill pre-approves the listed tools for the invoking turn so they don't prompt; it never restricts — every tool stays callable under the user's permission settings. List what the workflow's own instructions call (a skill that says "invoke `Skill:`" or "spawn via Agent" lists `Skill` / `Agent, Task`). `prototype` and `iterate` omit it: open-ended work has no bounded list worth pre-approving. `/praxis:implement` is the parallel orchestration command. There is no shipping or branch-cleanup command: commits, PRs, and stale-branch sweeps are handled directly.
...
- Skills activate automatically based on their `description` field; that description is effectively the trigger condition. Skills include ideate, prototype, iterate, and observe-before-model alongside the original set.
...
- **Lean mandate:** user-stated outcomes are fixed, machinery is variable; minimum competent implementation; crash-loud for impossible states; defensive code needs scenario/likelihood/consequence; cut-proposals are first-class review findings (raw boundary capture excepted — see Observe before model); tests assert behavior (inputs/state → observable output/effect), never source text or shape. Both pipelines carry an explicit trim pass (the `praxis:trimmer` agent): `/praxis:design` Phase 3 on the plan, `/praxis:implement` Phase 4 on the merged diff. Canonical statement in `skills/design/SKILL.md`; rationale in `plans/lean-mandate.md` (a local, untracked design note).
- **Observe before model:** boundary shapes come from the vendor's current docs plus one captured real call, never memory; raw request/response capture at a boundary is permanent and never a trim target (the named-reader test applies to derived instrumentation); reachable failures propagate with raw context, and a catch needs a named recovery beyond scenario/likelihood/consequence; design's flow sketch tags each boundary's shape source and Task 1 is a Slice 0 through the unobserved ones, which `/praxis:implement` runs before parallel dispatch; plan vocabulary never reaches code or comments (that rule lives in `skills/design/SKILL.md`'s Quality Rules). Canonical statement for the boundary discipline in `skills/observe-before-model/SKILL.md`; rationale in `plans/observe-before-model.md` (a local, untracked design note).
