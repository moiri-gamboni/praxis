...
- **`/praxis:design` Phase 1.2 (shared exploration wave)**: parallel with other instances, each focused on one dimension (architectural fit / touchpoints / risks-dependencies / constraints / failure modes). Write findings to `plans/<slug>/.workspace/exploration/<dimension>.md`; return summary + path.
